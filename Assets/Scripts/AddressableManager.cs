...
public abstract class AddressableManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        var firstAssetLoad = Addressables.LoadAssetAsync<MyRemoteChecker>(REMOTE_CHECKER_KEY);
        currentAsyncOperation = firstAssetLoad;
        firstAssetLoad.Completed += delegate(AsyncOperationHandle<MyRemoteChecker> handle)
        {
